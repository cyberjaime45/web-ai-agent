...
Flows are Markdown files (any `.md` pytest traverses; `tests/<app>/flows/` by convention), discovered by pytest, and
...
pytest tests/_framework                           # runtime self-tests only (no browser, ~1 s)
...
| `app/layers/providers/` | `LLMProvider` ABC, `get_provider()` factory (`AI_PROVIDER` empty → L3 off; set → key+model required), OpenAI / Gemini / Claude adapters |
| `app/flow/parser.py` | Markdown → `FlowDefinition(name, title, markers, section_markers, timeout, actions)`; 4-stage step pipeline; `resolve_flow_path` |
...
| `tests/_framework/` | Runtime self-tests. `tests/<app>/flows/` — flows per application (`marketing_site`, `members_site`, `fms`), shared sub-flows in `components/` |
...
`SLOW_MO` (0) · `AI_PROVIDER` (empty disables L3; when set, `LLM_KEY` + `LLM_MODEL` are required) · `REPORT_REDACT`.
...
- L3 is optional. Every flow must pass with no provider; an empty `AI_PROVIDER` disables L3, and a named provider without `LLM_KEY`/`LLM_MODEL` is a startup `UsageError`. Only actions in `_LOCATOR_ACTIONS` are sent to L3 (`AIResolver.supports`) — never let an assertion be "resolved" by the LLM.
...
- Framework tests never depend on the developer's `.env`: pass `provider=None` to `FlowRunner`, `monkeypatch.delenv`, fake pages. Keep `tests/_framework` under a few seconds.
- Minimal impact, root causes, no temporary fixes. Verify before marking done: `pytest tests/_framework`, a real flow run when the runtime changed, report opens with no console errors (Playwright headless check), `ruff` clean.
